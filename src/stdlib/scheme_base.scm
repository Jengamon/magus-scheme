...
(begin
  ...
  (define (length xs)
    ...
    (if (not (list? xs)) (error "length expects a list as its argument"))
    (length-iter xs 0))
  ...
  (define (make-list n . fill)
    ...
    (if (or (not (exact-integer? n)) (> 0 n)) (error "make-list expects a positive or zero exact integer as its first argument"))
    (if (> (length fill) 1) (error "make-list expects 1 or 2 arguments"))
    (if (> n 0)
        (make-list-iter '() n (if (null? fill) 0 (car fill)))
        '()))
  (define (list-ref list k)
      (define (list-ref-impl list idx)
          (if (not (pair? list)) (error (string-append "list-ref: index " (number->string k) " out of bounds")))
          (if (= idx 0)
              (car list)
              (list-ref-impl (cdr list) (- idx 1))))
      (if (not (pair? list)) (error "list-ref expects a pair as its first argument"))
      (if (not (and (exact-integer? k) (>= k 0))) (error "list-ref expects an exact nonnegative integer as its second argument"))
  ...
  (define (memq x lst)
      ...
      (memq-iter x lst))
  (define (assq x alist)
      (define (assp-check p) (if (not (or (null? p) (pair? (car p)))) (error "assq expects an association list as a second argument")))
  ...
  (define (memv x lst)
      ...
      (memv-iter x lst))
  (define (assv x alist)
      (define (assp-check p) (if (not (or (null? p) (pair? (car p)))) (error "assv expects an association list as a second argument")))
