...
// NOTE It's `Compiler::define_transformer_placeholder`
#[derive(Debug)]
...
pub struct SyntaxRulesImpl {
    ...
    branches: Vec<(Box<[Matcher]>, (Template, Option<SourceData>))>,
}
...
impl<'gc> Transformer<'gc> for SyntaxRulesImpl {
    fn evaluate(
        ...
        let code = compiler.hygenic_with_env(ctx, self.source_env, |ctx, compiler, import_env| {
            ...
            code.push(Bytecode::PushNull); // TODO go through branches, and find the first successful branch. take the matcher output(s) and merge it into
            // a binding that we can pass to a Template to get a ProgramPtr that we can then compile into code.
            // Wrap the resulting code into a chunk and install it as a macro chunk, using the requested macro invocation index.
            code
        });
...
#[collect(require_static)]
enum Matcher {
    // `_` is special, as it *can* show up multiple times in a pattern and *cannot* produce a value when matched (unlike variables)
    Underscore,
...
impl Syntax for SyntaxRules {
    fn evaluate<'gc>(
        ...
        // This identifies this specific invocation of `syntax-rules` so any code that it runs will run in the same
        // local environment (the env is not declared ready until a ReserveMacroEnv is emitted, where all invocations use
        // the env at that location as the parent environment for all invocations of that macro).
        let macro_env_id = compiler.request_macro_env_id();
        ...
        // IMPORTANT!! neither side is *actually* compiled at this point (into bytecode). we create something like
        // an IR that tells us (Matcher) how to search for and input code (Template) what the shape of the program output should be.
        // Invoking *that* syntax will then actually run the matcher-template to generate a Program from input
        // that is then compiled, and put into the chunk as a macro chunk to run on the thread.
